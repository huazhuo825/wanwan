...
function buildAskBoxSystemPrompt(char) {
  ...
  return '你正在扮演角色【' + name + '】。以下是你的完整人设：\n\n' + persona + '\n\n' +
    ...
    '5. 接下来给你的历史记录里，每条消息都会标注来源：标"[私聊]"的是你和某个人之间私下的微信聊天，标"[提问箱]"的是之前公开问答过的内容。' +
    '你可以参考私聊内容让自己更懂对方、回答更自然，但绝不能在这次公开回复里逐字复述或点名透露私聊的具体细节（比如对方说过的隐私、约定、私密话题），' +
    '除非那些内容本来就不涉及隐私。标"[提问箱]"的内容本来就是公开的，可以自然地延续或呼应。'
}

// ===== 上下文互通：合并"私聊原始消息"与"提问箱历史问答"，按时间顺序拼成带来源标注的多轮上下文 =====
// 目的：让提问箱内部的提问之间能接上上下文，也能感知私聊最近聊了什么；
// 注意：这里只是查询时临时拼装、传给 AI 做参考，不会把提问箱内容写回 db.messages，
// 避免匿名问答混进私聊的聊天气泡列表。
async function getAskBoxContextMessages(chatId, charId, historyLimit) {
  var limit = Math.max(4, Number(historyLimit) || 20)
  var wechatMsgs = []
  try {
    wechatMsgs = (await db.messages.where('chatId').equals(chatId).sortBy('createdAt')).slice(-limit)
  } catch (e) {
    console.warn('[askbox] 读取私聊历史失败：', e)
  }
  var askItems = []
  try {
    var all = await getAskBoxItems()
    askItems = all.filter(function(i) { return String(i.charId) === String(charId) && i.answer }).slice(-limit)
  } catch (e) {
    console.warn('[askbox] 读取提问箱历史失败：', e)
  }

  var merged = []
  wechatMsgs.forEach(function(m) {
    if (m.role !== 'user' && m.role !== 'assistant') return
    if (!String(m.content || '').trim()) return
    merged.push({ role: m.role, content: String(m.content), createdAt: m.createdAt || 0, source: 'wechat' })
  })
  askItems.forEach(function(i) {
    merged.push({ role: 'user', content: i.question, createdAt: i.createdAt || 0, source: 'askbox' })
    merged.push({ role: 'assistant', content: i.answer, createdAt: (i.createdAt || 0) + 1, source: 'askbox' })
  })
  merged.sort(function(a, b) { return a.createdAt - b.createdAt })
  merged = merged.slice(-limit)

  return merged.map(function(m) {
    var prefix = m.source === 'askbox'
      ? (m.role === 'assistant' ? '[提问箱] 你之前公开回复过：' : '[提问箱] 匿名网友之前问过：')
      : (m.role === 'assistant' ? '[私聊] 你之前私下回复过：' : '[私聊] 对方私下说过：')
...
async function submitAskBoxQuestion(charId, question, page, chars) {
  ...
  var item = {
    ...
    error: '',
    createdAt: Date.now(),
    summarized: false // 是否已被合并进长期记忆总结批次，由 memory.js 的 runSummary 回写
  }
  var items = await getAskBoxItems()
  items.push(item)
  ...
  try {
    ...
    // 互通第一步（压缩后的长期记忆）：先把角色和这位用户之间已经积累的长期记忆
    ...
    // 互通第二步（原始上下文）：把最近的私聊消息 + 提问箱历史问答，按时间合并成带来源标注的多轮上下文，
    // 让提问箱内部的多次提问能接上、也能感知私聊最近聊了什么（详见 buildAskBoxSystemPrompt 里的隐私边界规则）。
    var contextMessages = []
    if (memCtx) {
      try {
        var historyLimit = (typeof getChatHistoryLimit === 'function')
          ? await getChatHistoryLimit(memCtx.chatId)
          : 20
        contextMessages = await getAskBoxContextMessages(memCtx.chatId, charId, historyLimit)
      } catch (e) {
        console.warn('[askbox] 拼装上下文失败，退化为单轮：', e)
      }
    }
    contextMessages.push({ role: 'user', content: '[提问箱] 匿名网友现在问：' + question })

    var reply = await fetchAI(cfg, contextMessages, {
      system: systemPrompt,
  ...
  // 互通第三步：不再单独总结。这条问答会被同一套私聊总结计数器（summarizeIfNeeded）
  // 与私聊消息合并计数，攒够阈值后一并触发一次总结，sourceType 按批次构成自动标为
  // wechat / askbox / mixed。全程不阻塞 UI，失败不影响提问箱正常使用。
  if (item.answer && memCtx && window.WanWanMemory && window.WanWanMemory.summarizeIfNeeded) {
    setTimeout(function() {
      window.WanWanMemory.summarizeIfNeeded(memCtx.chatId, charId, memCtx.ownerUid).catch(function(e) {
