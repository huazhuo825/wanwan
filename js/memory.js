...
(function() {
  ...
  function buildSummaryPrompt(messages) {
    var hasAskbox = messages.some(function(m) { return m.source === 'askbox' })
    var lines = messages.map(function(m) {
      var fromAskbox = m.source === 'askbox'
      var speaker = m.role === 'assistant'
        ? (fromAskbox ? '角色（在匿名提问箱公开回复）' : '角色')
        : (fromAskbox ? '匿名网友（在提问箱提问）' : '用户')
      return speaker + '：' + String(m.content || '').replace(/\s+/g, ' ').slice(0, 800)
    }).join('\n')
    return `你是一个长期记忆整理器。请根据下面的记录，提取适合长期保存的记忆。
...
5. 禁止加入记录中没有出现的信息。
6. 标题应尽量简短；内容应控制在150字以内，适合未来角色回复时参考。${hasAskbox ? '\n7. 记录中标注为“匿名提问箱”的部分是公开问答，提问者身份对角色不可见，且角色的回复也是公开的；请在记忆内容里保留这个来源区别（例如“有匿名网友问……角色公开回应……”），不要把它写成私聊对话，也不要把提问者当成固定的聊天对象。' : ''}
...
${lines}`
  ...
  // 读取该角色在提问箱里"还没被归档进长期记忆"的问答，转换成参与总结的伪消息。
  // 不落库进 db.messages（避免混进私聊气泡列表），只在总结/上下文拼装时临时读取。
  async function getPendingAskBoxAsMessages(charId) {
    try {
      var row = await db.config.get('askBoxItems')
      var items = (row && Array.isArray(row.value)) ? row.value : []
      var pending = items.filter(function(i) {
        return String(i.charId) === String(charId) && String(i.answer || '').trim() && !i.summarized
      })
      var msgs = []
      pending.forEach(function(i) {
        var base = isValidTimestamp(i.createdAt) ? Number(i.createdAt) : Date.now()
        msgs.push({ askboxItemId: i.id, role: 'user', content: i.question, source: 'askbox', createdAt: base })
        msgs.push({ askboxItemId: i.id, role: 'assistant', content: i.answer, source: 'askbox', createdAt: base + 1 })
      })
      return msgs
    } catch (e) {
      console.warn('[memory] 读取待总结提问箱记录失败：', e)
      return []
    }
  }

  async function markAskBoxItemsSummarized(itemIds) {
    ...
    try {
      ...
      items.forEach(function(i) {
        if (idSet[i.id] && !i.summarized) { i.summarized = true; changed = true }
      })
      if (changed) await db.config.put({ key: 'askBoxItems', value: items })
    } catch (e) {
      console.warn('[memory] 标记提问箱记录已总结失败：', e)
    }
  }
  ...
  async function runSummary(chatId, charId, ownerUid, options) {
    ...
    var freshWechat = msgs.filter(function(m) { return m.id > settings.lastSummarizedMessageId })
      .map(function(m) { return Object.assign({}, m, { source: 'wechat' }) })
    var freshAskbox = await getPendingAskBoxAsMessages(charId)
    var fresh = freshWechat.concat(freshAskbox).sort(function(a, b) { return (a.createdAt || 0) - (b.createdAt || 0) })
    if (!fresh.length) {
      return { ok: false, skipped: true, reason: '没有可总结的新内容', memoryCount: 0, messageCount: 0 }
    }
    ...
    var hasWechat = freshWechat.length > 0
    var hasAskbox = freshAskbox.length > 0
    var batchSourceType = hasWechat && hasAskbox ? 'mixed' : (hasAskbox ? 'askbox' : 'wechat')
    var wechatIds = freshWechat.map(function(m) { return m.id }).filter(function(id) { return id != null })
    var fromMsgId = wechatIds.length ? Math.min.apply(null, wechatIds) : 0
    var toMsgId = wechatIds.length ? Math.max.apply(null, wechatIds) : 0
    ...
    for (var i = 0; i < memories.length; i++) {
      ...
      if (!row) continue
      row.sourceType = batchSourceType
    ...
    if (hasWechat) await saveSettings(chatId, { lastSummarizedMessageId: toMsgId })
    if (hasAskbox) {
      var summarizedAskboxIds = []
      freshAskbox.forEach(function(m) {
        if (summarizedAskboxIds.indexOf(m.askboxItemId) < 0) summarizedAskboxIds.push(m.askboxItemId)
      })
      await markAskBoxItemsSummarized(summarizedAskboxIds)
    }
  ...
  // 供私聊（含语音/视频通话）实时感知"提问箱里刚发生、还没被归档进长期记忆"的动态。
  // 一旦这些问答被 runSummary 合并总结、标记 summarized=true，就会自动从这里消失，不会和长期记忆重复。
  async function getAskBoxRecentPulse(charId) {
    try {
      var row = await db.config.get('askBoxItems')
      var items = (row && Array.isArray(row.value)) ? row.value : []
      var pending = items.filter(function(i) {
        return String(i.charId) === String(charId) && String(i.answer || '').trim() && !i.summarized
      }).sort(function(a, b) { return (a.createdAt || 0) - (b.createdAt || 0) }).slice(-8)
      if (!pending.length) return ''
      var lines = pending.map(function(item, idx) {
        var q = String(item.question || '').replace(/\s+/g, ' ').slice(0, 200)
        var a = String(item.answer || '').replace(/\s+/g, ' ').slice(0, 200)
        return (idx + 1) + '. [' + formatMemoryDateTime(item.createdAt) + '] 匿名网友问：' + q + '　你（公开）回复：' + a
      }).join('\n')
      return '\n\n# 最近的匿名提问箱动态（尚未整理进长期记忆，仅供参考）\n' +
        '以下是最近有人在匿名提问箱公开问你、你已公开回复过的内容。提问者身份对你不可见，这些回复也是公开可见的，' +
        '不代表和当前聊天对象之间发生过的私下对话，不要把它当成私聊记忆混用，只需自然知晓即可：\n' + lines
    } catch (e) {
      console.warn('[memory] 获取提问箱近况失败：', e)
      return ''
    }
  }
  ...
  function buildMemoryCard(m, chars) {
    ...
    var sourceLabel = m.sourceType === 'offlineMeet' ? '见面'
      ...
      : m.sourceType === 'mixed' ? '微信+提问箱'
      : '微信'
    var sourceClass = sourceLabel === '见面' ? 'is-meet' : (sourceLabel === '提问箱' || sourceLabel === '微信+提问箱') ? 'is-askbox' : 'is-wechat'
    return `
  ...
  window.WanWanMemory = {
    ...
    getAskBoxRecentPulse: getAskBoxRecentPulse,
